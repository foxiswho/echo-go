...
func NewOrderGoodsStructureService() *OrderGoodsStructureService {
	return new(OrderGoodsStructureService)
}

//初始化列表
func orderGoodsStructureNewMakeDataArr() []models.OrderGoodsStructure {
	return make([]models.OrderGoodsStructure, 0)
}
...
func (s *OrderGoodsStructureService) GetAll(where []*db.QueryCondition, fields []string, orderBy string, page int, limit int) (*db.Paginator, error) {
	m := models.NewOrderGoodsStructure()
	...
	data := orderGoodsStructureNewMakeDataArr()
...
// 获取 单条记录
func (s *OrderGoodsStructureService) GetById(id int) (*models.OrderGoodsStructure, error) {
    m:=new(models.OrderGoodsStructure)
	m.Id = id
